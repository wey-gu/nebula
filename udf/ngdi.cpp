...
nebula::Value ngdi::call_ngdi_api(
  ...
  if (read_mode != read_context.end() && read_mode->second.getStr() == "query") {
    ...
    if (query == read_context.end()) {
      nebula::Map response;
      response.emplace("error",
                       nebula::Value("Invalid read_context: no query found in read_mode: query"));
      return nebula::Value(response);
    }
  ...
  auto response_str =
      HttpClient::instance().post(url_prefix + mode + "/" + algo_name_value, headers, body_str);
  ...
  nebula::Value ngdi::body(const std::vector<std::reference_wrapper<const nebula::Value>> &args) {
    ...
    // validate the first arg is a string, if not, return response MAP with error message: "Invalid
    // args[0]: {args[0]}"
    ...
    if (args[1].get().type() == nebula::Value::Type::LIST) {
      ...
      for (auto i = 0; i < args[1].get().size(); i++) {
        if (args[1].get()[i].type() != nebula::Value::Type::STRING ||
            args[2].get()[i].type() != nebula::Value::Type::STRING) {
          nebula::Map response;
          response.emplace("error",
                           nebula::Value("Invalid args[1]: edge_types and edge_weights should be a "
                                         "LIST of STRING in same size"));
